...
import Back from 'components/frontend/Back';
import Favorites from 'components/frontend/Favorites';
...
function Nav({ data, className, showSearch = true, showBack = false }) {
  const hadnleScroll = () => {
    ...
    if (document.querySelector('header')) {
      if (document.querySelector('header').classList.contains('cover')) {
        breakpoint = document.querySelector('header').clientHeight;
      }
    }
    if (y > breakpoint) {
            ...
            <div className="flex justify-between">
              <div className="flex space-x-4 md:space-x-10 items-center links-block">
                {data?.social?.instagram && (
                  <Link href={`https://${data.social.instagram}`}>
