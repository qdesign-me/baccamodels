export default {
  all: {
    info: {
      video: '/video/russia/home.mp4',
    },
  },
  russia: {
    contacts: {
      phone: '+1 212 334 7480',
      email: 'info@womenmanagement.com',
      address: ` 55 Hudson Yards
      <br />
      3rd Floor
      <br /> New York NY 10001
      <br />
      United States`,
    },
    texts: {
      intro: `<p> Women Management, one of the most influential and trusted fashion talent agencies in the world, was founded in New York in 1988. Celebrating inclusivity, diversity and empowerment, our teams constantly strive to provide exceptional management to our talents and outstanding service to our clients. Our approach always combines creativity, technology and business development aligned with respect, trust, humanity, ethics and social responsibility. Women is a forward looking organization always searching for new and innovative ways to adapt to and take advantage of the ever-changing landscape of the modeling industry while balancing an elevated image with financial success.</p>`,
      about: `<p>Women Management, one of the most influential and trusted fashion talent agencies in the world, was founded in New York in 1988. Celebrating inclusivity, diversity and empowerment, our teams constantly strive to provide exceptional management to our talents and outstanding service to our clients. Our approach always combines creativity, technology and business development aligned with respect, trust, humanity, ethics and social responsibility. Women is a forward looking organization always searching for new and innovative ways to adapt to and take advantage of the ever-changing landscape of the modeling industry while balancing an elevated image with financial success.</p><p>Women represents some of the world’s most renowned fashion talents including Mariacarla Boscono, Isabeli Fontana, Hanne Gaby, Josephine Le Tutour, Lais Ribeiro, Coco Rocha and Iris Strubegger among others. Focusing on the future, Women is consistently developing the next generation of modeling superstars including Valerie Scherzinger, Sacha Quenby, Madeleine Fisher, Penelope Ternes, Maike Inga, Mathilde Henning and Maryel Uchida to name a few.</p><p>Women is part of Elite World Group, the world’s foremost talent media agency. Our global footprint includes offices in New York, Los Angeles, Paris and Milan.</p>`,
    },
    info: {
      video: '/video/russia/home.mp4',
      social: {
        instagram: 'https:/instagram.com/baccamodels/',
        facebook: 'https://facebook.com/baccamodels/',
        vkontakte: 'https://vkontakte.com/baccamodels/',
      },
    },
    become: {
      img: '/images/become.jpg',
      text: `Women Management is constantly looking for new faces. Our expectations are as varied as the number of models we represent. You are in the right place if you want to
      give you this adventure a try and maybe become our future tops !`,
    },
    latest: [
      {
        img: '/images/russia/slides/1.jpg',
        model: 1,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/2.jpg',
        model: 2,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/3.jpg',
        model: 3,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/4.jpg',
        model: 4,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/1.jpg',
        model: 5,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/2.jpg',
        model: 6,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
    ],
    models: {
      women: [
        {
          id: 1,
          name: 'Adele Ruboneka',
          slug: '/russia/women/adele-ruboneka',
          img: '/images/russia/models/1.jpg',
          profile: {
            img: '/images/russia/models/1/book/1.jpg',
            book: [
              '/images/russia/models/1/book/1.jpg',
              '/images/russia/models/1/book/2.jpg',
              '/images/russia/models/1/book/3.jpg',
              '/images/russia/models/1/book/4.jpg',
              '/images/russia/models/1/book/5.jpg',
              '/images/russia/models/1/book/6.jpg',
              '/images/russia/models/1/book/1.jpg',
              '/images/russia/models/1/book/2.jpg',
              '/images/russia/models/1/book/3.jpg',
            ],
            polaroids: [
              '/images/russia/models/1/polaroids/1.jpg',
              '/images/russia/models/1/polaroids/2.jpg',
              '/images/russia/models/1/polaroids/3.jpg',
              '/images/russia/models/1/polaroids/4.jpg',
            ],
            videos: [
              { img: '/images/russia/models/1/videos/1.jpg', url: '/video/home.mp4' },
              { img: '/images/russia/models/1/videos/2.jpg', url: '/video/home.mp4' },
            ],
            params: {
              Height: '180',
              Hair: 'Black',
              Eyes: 'Blue-green',
              Bust: '77',
              Waist: '58',
              Hips: '86',
              Shoes: '39',
            },
          },
        },
        {
          id: 2,
          slug: '/russia/women/allexia-de-jesus',
          name: 'Allexia De Jesus',
          img: '/images/russia/models/2.jpg',
        },
        {
          id: 3,
          slug: '/russia/women/bell-hinova',
          name: 'Bell Hinova',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 4,
          slug: '/russia/women/celina-krohn',
          name: 'Celina Krohn',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 5,
          slug: '/russia/women/chiara-bimbatti',
          name: 'Chiara Bimbatti',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 6,
          slug: '/russia/women/clea-beuret',
          name: 'Clea Beuret',
          img: '/images/russia/models/5.jpg',
        },
        {
          id: 7,
          slug: '/russia/women/dee-glover',
          name: 'Dee Glover',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 8,
          slug: '/russia/women/dija-kallon',
          name: 'Dija Kallon',
          img: '/images/russia/models/1.jpg',
        },
        {
          id: 9,
          slug: '/russia/women/elisa-löehr',
          name: 'Elisa Löehr',
          img: '/images/russia/models/2.jpg',
        },
      ],
      development: [
        {
          id: 10,
          slug: '/russia/development/emma-todt',
          name: 'Emma Todt',
          img: '/images/russia/models/5.jpg',
        },
        {
          id: 11,
          slug: '/russia/development/anna-berher',
          name: 'Anna Berher',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 12,
          slug: '/russia/development/olga-glover',
          name: 'Olga Glover',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 13,
          slug: '/russia/development/gu-haizhu',
          name: 'Gu Haizhu',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 14,
          slug: '/russia/development/gwen-weijers',
          name: 'Gwen Weijers',
          img: '/images/russia/models/5.jpg',
        },
      ],
      talent: [
        {
          id: 15,
          slug: '/russia/talent/pleun-keijsers',
          name: 'Pleun Keijsers',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 16,
          slug: '/russia/talent/inetria',
          name: 'Vinetria',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 17,
          slug: '/russia/talent/xuwei-chen',
          name: 'Xuwei Chen',
          img: '/images/russia/models/2.jpg',
        },
        {
          id: 18,
          slug: '/russia/talent/sia-vlasova',
          name: 'Sia Vlasova',
          img: '/images/russia/models/1.jpg',
        },
      ],
    },
  },
  kazakhstan: {
    introtext: `<p>
    kazakhstan text.
  </p>`,
    info: {
      video: '/video/russia/home.mp4',
      instagram: 'https://www.instagram.com/baccamodels/',
    },
    latest: [
      {
        img: '/images/russia/slides/1.jpg',
        model: 1,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/2.jpg',
        model: 2,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/3.jpg',
        model: 3,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/4.jpg',
        model: 4,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/1.jpg',
        model: 5,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/2.jpg',
        model: 6,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
    ],
    models: {
      women: [
        {
          id: 1,
          name: 'Adele Ruboneka',
          slug: '/russia/women/adele-ruboneka',
          img: '/images/russia/models/1.jpg',
          profile: {
            img: '/images/russia/models/1/book/1.jpg',
            book: [
              '/images/russia/models/1/book/1.jpg',
              '/images/russia/models/1/book/2.jpg',
              '/images/russia/models/1/book/3.jpg',
              '/images/russia/models/1/book/4.jpg',
              '/images/russia/models/1/book/5.jpg',
              '/images/russia/models/1/book/6.jpg',
              '/images/russia/models/1/book/1.jpg',
              '/images/russia/models/1/book/2.jpg',
              '/images/russia/models/1/book/3.jpg',
            ],
            polaroids: [
              '/images/russia/models/1/polaroids/1.jpg',
              '/images/russia/models/1/polaroids/2.jpg',
              '/images/russia/models/1/polaroids/3.jpg',
              '/images/russia/models/1/polaroids/4.jpg',
            ],
            videos: [
              { img: '/images/russia/models/1/videos/1.jpg', url: '/video/home.mp4' },
              { img: '/images/russia/models/1/videos/2.jpg', url: '/video/home.mp4' },
            ],
            params: {
              Height: '180',
              Hair: 'Black',
              Eyes: 'Blue-green',
              Bust: '77',
              Waist: '58',
              Hips: '86',
              Shoes: '39',
            },
          },
        },
        {
          id: 2,
          slug: '/russia/women/allexia-de-jesus',
          name: 'Allexia De Jesus',
          img: '/images/russia/models/2.jpg',
        },
        {
          id: 3,
          slug: '/russia/women/bell-hinova',
          name: 'Bell Hinova',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 4,
          slug: '/russia/women/celina-krohn',
          name: 'Celina Krohn',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 5,
          slug: '/russia/women/chiara-bimbatti',
          name: 'Chiara Bimbatti',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 6,
          slug: '/russia/women/clea-beuret',
          name: 'Clea Beuret',
          img: '/images/russia/models/5.jpg',
        },
        {
          id: 7,
          slug: '/russia/women/dee-glover',
          name: 'Dee Glover',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 8,
          slug: '/russia/women/dija-kallon',
          name: 'Dija Kallon',
          img: '/images/russia/models/1.jpg',
        },
        {
          id: 9,
          slug: '/russia/women/elisa-löehr',
          name: 'Elisa Löehr',
          img: '/images/russia/models/2.jpg',
        },
      ],
      development: [
        {
          id: 10,
          slug: '/russia/development/emma-todt',
          name: 'Emma Todt',
          img: '/images/russia/models/5.jpg',
        },
        {
          id: 11,
          slug: '/russia/development/anna-berher',
          name: 'Anna Berher',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 12,
          slug: '/russia/development/olga-glover',
          name: 'Olga Glover',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 13,
          slug: '/russia/development/gu-haizhu',
          name: 'Gu Haizhu',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 14,
          slug: '/russia/development/gwen-weijers',
          name: 'Gwen Weijers',
          img: '/images/russia/models/5.jpg',
        },
      ],
      talent: [
        {
          id: 15,
          slug: '/russia/talent/pleun-keijsers',
          name: 'Pleun Keijsers',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 16,
          slug: '/russia/talent/inetria',
          name: 'Vinetria',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 17,
          slug: '/russia/talent/xuwei-chen',
          name: 'Xuwei Chen',
          img: '/images/russia/models/2.jpg',
        },
        {
          id: 18,
          slug: '/russia/talent/sia-vlasova',
          name: 'Sia Vlasova',
          img: '/images/russia/models/1.jpg',
        },
      ],
    },
  },
  kids: {
    introtext: `<p>
    kids text.
  </p>`,
    info: {
      video: '/video/russia/home.mp4',
      instagram: 'https://www.instagram.com/baccamodels/',
    },
    latest: [
      {
        img: '/images/russia/slides/1.jpg',
        model: 1,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/2.jpg',
        model: 2,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/3.jpg',
        model: 3,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/4.jpg',
        model: 4,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/1.jpg',
        model: 5,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
      {
        img: '/images/russia/slides/2.jpg',
        model: 6,
        text: 'for Vogue Arabia by Jennifer Kalaitzis',
      },
    ],
    models: {
      women: [
        {
          id: 1,
          name: 'Adele Ruboneka',
          slug: '/russia/women/adele-ruboneka',
          img: '/images/russia/models/1.jpg',
          profile: {
            img: '/images/russia/models/1/book/1.jpg',
            book: [
              '/images/russia/models/1/book/1.jpg',
              '/images/russia/models/1/book/2.jpg',
              '/images/russia/models/1/book/3.jpg',
              '/images/russia/models/1/book/4.jpg',
              '/images/russia/models/1/book/5.jpg',
              '/images/russia/models/1/book/6.jpg',
              '/images/russia/models/1/book/1.jpg',
              '/images/russia/models/1/book/2.jpg',
              '/images/russia/models/1/book/3.jpg',
            ],
            polaroids: [
              '/images/russia/models/1/polaroids/1.jpg',
              '/images/russia/models/1/polaroids/2.jpg',
              '/images/russia/models/1/polaroids/3.jpg',
              '/images/russia/models/1/polaroids/4.jpg',
            ],
            videos: [
              { img: '/images/russia/models/1/videos/1.jpg', url: '/video/home.mp4' },
              { img: '/images/russia/models/1/videos/2.jpg', url: '/video/home.mp4' },
            ],
            params: {
              Height: '180',
              Hair: 'Black',
              Eyes: 'Blue-green',
              Bust: '77',
              Waist: '58',
              Hips: '86',
              Shoes: '39',
            },
          },
        },
        {
          id: 2,
          slug: '/russia/women/allexia-de-jesus',
          name: 'Allexia De Jesus',
          img: '/images/russia/models/2.jpg',
        },
        {
          id: 3,
          slug: '/russia/women/bell-hinova',
          name: 'Bell Hinova',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 4,
          slug: '/russia/women/celina-krohn',
          name: 'Celina Krohn',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 5,
          slug: '/russia/women/chiara-bimbatti',
          name: 'Chiara Bimbatti',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 6,
          slug: '/russia/women/clea-beuret',
          name: 'Clea Beuret',
          img: '/images/russia/models/5.jpg',
        },
        {
          id: 7,
          slug: '/russia/women/dee-glover',
          name: 'Dee Glover',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 8,
          slug: '/russia/women/dija-kallon',
          name: 'Dija Kallon',
          img: '/images/russia/models/1.jpg',
        },
        {
          id: 9,
          slug: '/russia/women/elisa-löehr',
          name: 'Elisa Löehr',
          img: '/images/russia/models/2.jpg',
        },
      ],
      development: [
        {
          id: 10,
          slug: '/russia/development/emma-todt',
          name: 'Emma Todt',
          img: '/images/russia/models/5.jpg',
        },
        {
          id: 11,
          slug: '/russia/development/anna-berher',
          name: 'Anna Berher',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 12,
          slug: '/russia/development/olga-glover',
          name: 'Olga Glover',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 13,
          slug: '/russia/development/gu-haizhu',
          name: 'Gu Haizhu',
          img: '/images/russia/models/6.jpg',
        },
        {
          id: 14,
          slug: '/russia/development/gwen-weijers',
          name: 'Gwen Weijers',
          img: '/images/russia/models/5.jpg',
        },
      ],
      talent: [
        {
          id: 15,
          slug: '/russia/talent/pleun-keijsers',
          name: 'Pleun Keijsers',
          img: '/images/russia/models/4.jpg',
        },
        {
          id: 16,
          slug: '/russia/talent/inetria',
          name: 'Vinetria',
          img: '/images/russia/models/3.jpg',
        },
        {
          id: 17,
          slug: '/russia/talent/xuwei-chen',
          name: 'Xuwei Chen',
          img: '/images/russia/models/2.jpg',
        },
        {
          id: 18,
          slug: '/russia/talent/sia-vlasova',
          name: 'Sia Vlasova',
          img: '/images/russia/models/1.jpg',
        },
      ],
    },
  },
};
